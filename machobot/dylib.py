...
def insert_load_command(target_path, library_install_name):
    ...
    Note: the target file will be overwritten. Consider backing it up first
...
def macho_dependencies_list(target_path, header_magic=None):
    ...
    libraries it would be a full file path, and for others it would be a
    relative path (sometimes with dyld templates like @executable_path or
    @rpath in it).

    Note: I don't know any reason why would some architectures of a fat Mach-O
    depend on certain libraries while others don't, but
    *it's technically possible*.
    So that's why you may want to specify the `header_magic` value for a
    particular header.

    Returns an object with two properties: `weak` and `strong` that hold
    ...
    def strongReferencesFromHeader(h):
        ...
        list = [
            lc_cmd_data for lc_cmd_data in h.commands
            if lc_cmd_data[0].lc_cmd_data[1] == LC_LOAD_DYLIB]
        # Their contents (aka data) as a file path
        return [decodeLoadCommandData(lc_cmd_data1[2]) for lc_cmd_data1 in list]

    def weakReferencesFromHeader(h):
        list = [
            lc_cmd_data2 for lc_cmd_data2 in h.commands
            if lc_cmd_data2[0].lc_cmd_data2[1] == LC_LOAD_WEAK_DYLIB]
        return [decodeLoadCommandData(lc_cmd_data3[2]) for lc_cmd_data3 in list]
...
def generate_dylib_load_command(header, libary_install_name):
    """
    Generates a LC_LOAD_DYLIB command for the given header and a
    library install path.

    Note: the header must already contain at least one LC_LOAD_DYLIB command
    (see code comments).
    Returns a ready-for-use load_command in terms of macholib.
    """

    # One can not simply create instances of `dylib_command` and `load_command`
    # classes, because that's just not the way macholib works.
    # If we try then all we'll get is a bunch
    # of endian (big/little) issues when these objects are serialized into a
    # file.
    # BUT THAT'S PROGRAMMING RIGHT?
    # So instead I'll iterate *existing* load commands, find a dyld_command,
    # copy it and modify this copy. This existing command is said to be
    # fully initialized.
    lc = None
    cmd = None
    for (command, internal_cmd, data) in header.commands:
        if (command.cmd == LC_LOAD_DYLIB) and \
                isinstance(internal_cmd, dylib_command):
            lc = deepcopy(command)
            cmd = deepcopy(internal_cmd)
            break
    if not lc or not cmd:
        raise Exception(
            "Invalid Mach-O file. I mean, there must be at least one "
    ...
    # Since we store the library's path just after the load command itself,
    # we need to find out it's offset.
    base = sizeof(load_command) + sizeof(dylib_command)
    # `name` is rather bad name for this property:
    # actually it means a path string offset
    cmd.name = base
    # Also the whole thing must be aligned by 4 bytes on 32-bit
